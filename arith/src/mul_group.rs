...
#[derive(Debug, Clone)]
pub struct Radix2Group<F: TwoAdicField> {
    log_order: u32,
    omega: F,
    elements: Rc<Vec<F>>,
}

impl<F: TwoAdicField> Radix2Group<F> {
...
mod tests {
    ...
    fn fft_and_ifft() {
        ...
        for _i in 0..16 {
            a.push(Fr::random_unsafe(&mut rng));
            b.push(Fr::random_unsafe(&mut rng));
        }
        for _i in 16..32 {
            a.push(Fr::zero());
            b.push(Fr::zero());
    ...
    #[test]
    fn elements() {
        let coset = Radix2Group::<Fr>::new(5);
